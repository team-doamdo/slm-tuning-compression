...
def measure_head_activation(model, tokenizer, data_samples):
    """
    Hidden states 기반으로 attention heads activation 측정
    ...
    Returns:
        head_outputs: Dict[layer_idx][head_idx] = [activation_values]
    """
    
    print(" Attention heads activation 측정 중...")
    ...
    # Hook: attention 출력에서 각 헤드별 기여도 측정
    def create_hook(layer_idx):
        def hook(module, input, output):
            try:
                # output[0]는 hidden_states [batch, seq_len, hidden_size]
                if isinstance(output, tuple):
                    hidden_states = output[0]
                else:
                    hidden_states = output
                
                # hidden_size를 head별로 분할
                batch_size, seq_len, hidden_size = hidden_states.shape
                
                # [batch, seq_len, num_heads, head_dim]로 reshape
                hidden_per_head = hidden_states.view(batch_size, seq_len, num_q_heads, head_dim)
                
                # 각 헤드의 평균 출력 크기 계산
                for head_idx in range(num_q_heads):
                    head_output = hidden_per_head[:, :, head_idx, :].abs().mean().item()
                    head_outputs[layer_idx][head_idx].append(head_output)
    ...
    for layer_idx in range(num_layers):
        try:
            ...
            print(f"⚠️ 레이어 {layer_idx} hook 등록 실패: {e}")
    ...
    print(f" Activation 측정 완료")
    print(f"   수집된 헤드: {collected_heads}개 (예상: {num_layers * num_q_heads}개)")
    
    return head_outputs


def calculate_head_importance(activations):
    """
    측정된 activation으로 중요도 계산
    
    Args:
        activations: measure_head_activation()의 출력
        
    Returns:
        head_scores: Dict[layer_idx][head_idx] = score
    """
    
    print(" Activation 기반 중요도 계산 중...")
    ...
    for layer_idx, layer_activations in activations.items():
        ...
        for head_idx, head_activation_list in layer_activations.items():
            if len(head_activation_list) > 0:
                # 평균 activation 값을 중요도로 사용
                importance = np.mean(head_activation_list)
...
if __name__ == "__main__":
    print("=" * 60)
    print(" Attention Heads Activation 중요도 분석")
    print("   (원본 모델 직접 프루닝)")
    print("=" * 60)
    ...
    if DEBUG_MODE:
        MAX_SAMPLES = 10
        print("⚠️ DEBUG_MODE 활성화 - 10개 샘플만 사용")
    ...
    try:
        ...
        # 3. Activation 측정
        activations = measure_head_activation(model, tokenizer, data_samples)
        
        # 4. 중요도 계산
        scores = calculate_head_importance(activations)
        ...
        print("\n" + "=" * 60)
        print(" Attention heads activation 분석 완료!")
